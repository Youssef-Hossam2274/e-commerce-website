    ...
    <Menu open={isMenuOpen} handler={setIsMenuOpen} placement="bottom-end">
      <MenuHandler>
        <Button
          variant="text"
          color="blue-gray"
          className="flex items-center gap-1 rounded-full py-0.5 pr-2 pl-0.5 lg:ml-auto dark:text-gray-200"
        >
          <Avatar
            variant="circular"
            size="sm"
          ...
          <ChevronDownIcon
            strokeWidth={2.5}
            className={`h-3 w-3 transition-transform ${isMenuOpen ? "rotate-180" : ""} dark:text-gray-400`}
          />
        </Button>
      </MenuHandler>
          ...
          return (
            ...
            >
              {React.createElement(icon, {
                className: `h-4 w-4 ${isLastItem ? "text-red-500" : "text-gray-600 dark:text-gray-400"}`,
                strokeWidth: 2,
              })}
              <Typography
                as="span"
  ...
  const renderItems = navListMenuItems.map(({ title, description }) => (
    <a href="#" key={title}>
      <MenuItem className="dark:hover:bg-gray-700">
        <Typography variant="h6" color="blue-gray" className="mb-1 dark:text-gray-200">
          {title}
        </Typography>
  ...
  return (
    <React.Fragment>
      <Menu allowHover open={isMenuOpen} handler={setIsMenuOpen}>
        <MenuHandler>
          <Typography as="a" href="#" variant="small" className="font-normal dark:text-gray-200">
        ...
        <MenuList className="hidden w-[36rem] grid-cols-7 gap-3 overflow-visible lg:grid dark:bg-gray-800">
          <Card color="blue" shadow={false} variant="gradient" className="col-span-3 grid h-full w-full place-items-center rounded-md">
            <RocketLaunchIcon strokeWidth={1} className="h-28 w-28 dark:text-gray-200" />
          </Card>
      ...
      <MenuItem className="flex items-center gap-2 font-medium text-blue-gray-900 dark:text-gray-200 lg:hidden">
        <Square3Stack3DIcon className="h-[18px] w-[18px] text-blue-gray-500 dark:text-gray-400" /> Pages{" "}
      </MenuItem>
      ...
      {navListItems.map(({ label, icon }, key) => (
        ...
        >
          <MenuItem className="flex items-center gap-2 lg:rounded-full dark:hover:bg-gray-700">
            {React.createElement(icon, { className: "h-[18px] w-[18px] dark:text-gray-400" })}{" "}
...
export function UserNavbar() {
  ...
  return (
    <nav className="mx-auto w-screen p-2 lg:pl-6 dark:bg-gray-800">
